...
var script = {
  ...
  methods: {
    ...
    getCycleMovement: function getCycleMovement(xPos, yPos, width, height, shape) {
      ...
      this.cycleMovement = { x: x, y: y, width: width, height: height };
    },
...
var transformMixin = {
  methods: {
    ...
    scaleMovement: function scaleMovement(x, y) {
      var type = this.type;
      var movement =
        (Math.sign(this.strength) * (Math.abs(x) + Math.abs(y))) / 10 + 1;
...
var script$1 = {
  ...
  methods: {
    transformMovement: function transformMovement() {
      ...
      var transform, movementX, movementY;
      ...
      if (this.cycle <= 0) {
        var ref = !this.isTouch ? context.movement : context.orientation;
        var x = ref.x;
        var y = ref.y;
        ...
        var ref$1 = context.cycleMovement;
        var x$1 = ref$1.x;
        var y$1 = ref$1.y;
      ...
      transformType =
        transformType === "scaleX" || transformType === "scaleY"
          ? "scale"
          : transformType;
...
var script$3 = {
  ...
  methods: {
    ...
    transformBehavior: function transformBehavior(shape) {
      var movementX, movementY, transform;
      var scrollPosition =
        (shape.top - window.innerHeight) / (shape.height + window.innerHeight);
      if (this.cycle <= 0) {
        ...
        movementX =
          this.axis === "x" ? (cycleX / (width / 2)) * this.strength : 0;
        movementY =
          this.axis === "y" || !this.axis
...
export { __vue_component__$2 as KinesisAudio, __vue_component__ as KinesisContainer, __vue_component__$4 as KinesisDistance, __vue_component__$1 as KinesisElement, __vue_component__$3 as KinesisScroll, install };
